...
describe("authenticated analysis procedures", () => {
  ...
  it("rejects a malformed report shape before touching the database", async () => {
    const caller = goldRouter.createCaller({ user } as any);
    await expect(caller.analysis.saveAiReport({ accountId: 44, filters: {}, model: "openai/gpt-4o-mini", report: { executiveSummary: "only a summary" } as never })).rejects.toMatchObject({ code: "BAD_REQUEST" });
