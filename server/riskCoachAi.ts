...
export const MAX_RISK_COACH_TIMEOUT_MS = 14 * 60_000;
...
export function resolveRiskCoachTimeoutMs(value = process.env.OPENROUTER_RISK_COACH_TIMEOUT_MS ?? process.env.OPENROUTER_TIMEOUT_MS) { const parsed = Number(value); if (!Number.isFinite(parsed)) return DEFAULT_RISK_COACH_TIMEOUT_MS; return Math.min(MAX_RISK_COACH_TIMEOUT_MS, Math.max(MIN_RISK_COACH_TIMEOUT_MS, Math.floor(parsed))); }
...
export async function coachRiskWithOpenRouter(userId: number, calculation: RiskCalculation): Promise<RiskCoachOutcome> {
  ...
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${credential.key}`, "HTTP-Referer": process.env.OPENROUTER_APP_URL?.trim() || "https://github.com/Najam27/gold-journal-", "X-Title": "Gold Journal Risk Coach" }, signal: controller.signal, body: JSON.stringify({ model: credential.model, temperature: 0, messages: [{ role: "system", content: system }, { role: "user", content: JSON.stringify(compact) }], response_format: { type: "json_schema", json_schema: { name: "gold_journal_risk_coach", strict: true, schema: responseSchema } } }) });
