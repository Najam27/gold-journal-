...
describe("risk coach safeguards", () => {
  it("uses the selected bounded two-minute default and permits longer background overrides", () => {
    expect(DEFAULT_RISK_COACH_TIMEOUT_MS).toBe(120_000); expect(resolveRiskCoachTimeoutMs(undefined)).toBe(120_000); expect(resolveRiskCoachTimeoutMs("240000")).toBe(240_000); expect(resolveRiskCoachTimeoutMs("5000")).toBe(5_000);
  });
