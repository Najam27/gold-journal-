...
function parseNormalizedBody(req: Request, next: NextFunction) {
  ...
  const normalized = req.body.toString("utf8").replace(/\u0000+$/g, "").trim();
  try {
    req.body = JSON.parse(normalized);
    next();
  } catch (error) {
    const parseError = error instanceof SyntaxError ? error : new SyntaxError("Malformed JSON request body.");
    Object.assign(parseError, { body: normalized, type: "entity.parse.failed" });
    next(parseError);
  }
}
...
export function mt5JsonBody(req: Request, res: Response, next: NextFunction) {
  return express.raw({ type: "application/json", limit: "256kb" })(req, res, error => {
