...
| `AI_KEY_ENCRYPTION_SECRET` | Netlify only, required for durable AI | A long random server-only secret used to encrypt each user's OpenRouter key before it is stored in Supabase; never use a `VITE_` prefix or commit this value. When omitted, the server derives a vault key from `SUPABASE_SERVICE_ROLE_KEY` — workable but fragile: rotating the Supabase key locks out every saved AI key. |
| `AI_JOB_WORKER_BASE_URL` | Netlify only, required for AI | The public origin of this deployment (for example `https://your-site.netlify.app`) that the tRPC API posts durable AI work to (`/.netlify/functions/ai-job-worker`). Netlify injects `URL`/`DEPLOY_PRIME_URL` only at build time, so set this explicitly; without it AI jobs stay QUEUED forever or fail with "AI background processing could not be started". |
| `NODE_ENV` | Netlify only | Netlify does not set `NODE_ENV` by default. Set it to `production` so the API validates server configuration and AI dispatch never silently uses the inline development fallback. |
| `AI_JOB_INLINE_FALLBACK` | Local only | Set to `true` only on a long-running Node process (`pnpm dev`) if you want AI jobs to run in-process. Never enable on Netlify Functions. |

Each user now manages their own OpenRouter key in **Options → Private AI Provider**. The browser sends it only to the authenticated server for verification and AES-256-GCM encryption; the database stores ciphertext, IV, authentication tag, fingerprint, mask, and selected model, never plaintext. Analysis, AI Mentor, and Risk Coach use that authenticated user's saved key and do not use shared `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, or fallback-model credentials. Each AI request queues secure work and polls its user-owned completion record; the included Netlify Background Function can run for up to fifteen minutes, avoiding the synchronous API/proxy response failure that affected long reports. Queued jobs that never start are marked FAILED after two minutes and stuck RUNNING jobs after sixteen minutes, so the UI always reaches a terminal state. `OPENROUTER_TIMEOUT_MS` (default 120 s, may be raised up to 14 minutes for background processing) and `OPENROUTER_APP_URL` remain optional runtime settings. Never create a `VITE_OPENROUTER_API_KEY` or place provider credentials in browser storage. The old `VITE_APP_ID`, `JWT_SECRET`, `OAUTH_SERVER_URL`, `VITE_OAUTH_PORTAL_URL`, `OWNER_OPEN_ID`, `OWNER_NAME`, and source Forge variables are not required by the Supabase Auth flow.
