...
There is **no server-side AI configuration**. Analysis, AI Mentor, and Risk Coach call OpenRouter directly from the user's browser using the user's own key, so Cloudflare never spends OpenRouter tokens and the backend never receives a provider credential.

1. In **Options → Private AI Provider**, paste your OpenRouter key and choose a model.
2. The key is stored in this browser's `localStorage` under `gold-journal.ai.openrouter:v1` and is read only by `client/src/lib/ai`.
3. Pressing **Analyze my journal** or **AI risk coach review** sends the request straight from the browser to `https://openrouter.ai`, then validates the response against the shared evidence schema before rendering it.
