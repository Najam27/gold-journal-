...
| `AI_KEY_ENCRYPTION_SECRET` | Netlify only | A long random server-only secret used to encrypt each user's OpenRouter key before it is stored in Supabase; never use a `VITE_` prefix or commit this value |

Each user now manages their own OpenRouter key in **Options → Private AI Provider**. The browser sends it only to the authenticated server for verification and AES-256-GCM encryption; the database stores ciphertext, IV, authentication tag, fingerprint, mask, and selected model, never plaintext. Analysis, AI Mentor, and Risk Coach use that authenticated user's saved key and do not use shared `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, or fallback-model credentials. `OPENROUTER_TIMEOUT_MS` and `OPENROUTER_APP_URL` remain optional runtime settings. Never create a `VITE_OPENROUTER_API_KEY` or place provider credentials in browser storage. If `AI_KEY_ENCRYPTION_SECRET` is absent, the Options UI blocks key entry and deterministic analysis/risk calculation remain available. The old `VITE_APP_ID`, `JWT_SECRET`, `OAUTH_SERVER_URL`, `VITE_OAUTH_PORTAL_URL`, `OWNER_OPEN_ID`, `OWNER_NAME`, and source Forge variables are not required by the Supabase Auth flow.
