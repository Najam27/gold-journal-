...
| `AI_KEY_ENCRYPTION_SECRET` | Server secret, required for durable AI | A long random server-only secret used to encrypt each user's OpenRouter key before it is stored in Supabase; never use a `VITE_` prefix or commit this value. When omitted, the server derives a vault key from `SUPABASE_SERVICE_ROLE_KEY` — workable but fragile: rotating the Supabase key locks out every saved AI key. |
| `AI_JOB_WORKER_BASE_URL` | Server, required for AI on Cloudflare | The canonical public origin of this deployment (custom domain, e.g. `https://app.yourdomain.com`). The API posts durable AI work to `<origin>/api/ai-job-dispatch` on the Worker. During `pnpm dev` leave blank; AI jobs then run in-process. |
| `NODE_ENV` | Server | `production` on deployed targets so the API validates server configuration and inline AI fallback stays disabled. |
| `AI_JOB_INLINE_FALLBACK` | Node only | Set to `true` on a long-running Node process if you want AI jobs in-process (`pnpm dev` does this implicitly via `NODE_ENV=development`). Never needed on Cloudflare Workers. |

Each user now manages their own OpenRouter key in **Options → Private AI Provider**. The browser sends it only to the authenticated server for verification and AES-256-GCM encryption; the database stores ciphertext, IV, authentication tag, fingerprint, mask, and selected model, never plaintext. Analysis, AI Mentor, and Risk Coach use that authenticated user's saved key and do not use shared `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, or fallback-model credentials. Each AI request queues secure work and polls its user-owned completion record; the Cloudflare Worker executes the job on `/api/ai-job-dispatch` and the atomic QUEUED→RUNNING claim prevents double processing. Queued jobs that never start are marked FAILED after two minutes and stuck RUNNING jobs after sixteen minutes, so the UI always reaches a terminal state. `OPENROUTER_TIMEOUT_MS` (default 120 s) and `OPENROUTER_APP_URL` remain optional runtime settings. Never create a `VITE_OPENROUTER_API_KEY` or place provider credentials in browser storage. The old `VITE_APP_ID`, `JWT_SECRET`, `OAUTH_SERVER_URL`, `VITE_OAUTH_PORTAL_URL`, `OWNER_OPEN_ID`, `OWNER_NAME`, source Forge variables, and Netlify's `URL`/`DEPLOY_PRIME_URL` are not required by the Supabase Auth flow.
