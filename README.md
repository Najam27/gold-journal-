...
After setting environment variables, apply all six Supabase migrations in order, then deploy the site. Verify account creation, email confirmation or magic-link return, password sign-in, sign-out, trade creation, screenshot upload, account clearing/removal, notification pagination/mark-all-read, and MT5 connection setup. Do not deploy code that calls the atomic or trade-summary RPCs before migrations `0004` and `0005` have been applied, and apply `0006` before relying on its database checks, notification uniqueness, or updatedAt triggers.
