...
import { PDF_MISSING, TRADE_PDF_INTERNAL_KEYS, TRADE_PDF_KPI_LABELS, TRADE_PDF_MAPPED_KEYS, buildTradePdfModel, checklistCompletion, checklistItems, checklistText, fieldTone, tradeIdLabel } from "./tradePdfModel";
...
describe("canonical trade PDF model", () => {
  it("exports every persisted trade field of a complete trade", () => {
    ...
    expect(model.sections.map(section => section.title)).toEqual(["Trade overview", "Strategy & execution", "Risk & performance", "Plan & discipline", "Process & mistakes"]);
    ...
    expect(value(model, "Open time (MT5)")).toBe("12 Aug 2026, 14:30");
    expect(value(model, "Close time (MT5)")).toBe("12 Aug 2026, 16:15");
    ...
    expect(value(model, "Mistake tags")).toBe("Early entry|Oversize");
  ...
  it("publishes the headline figures as a typed KPI strip in the report's order", () => {
    const model = buildTradePdfModel(completeTrade, { runningBalance: 1275.5 });
    expect(model.kpis.map(kpi => kpi.label)).toEqual(TRADE_PDF_KPI_LABELS);
    expect(model.kpis.map(kpi => kpi.value)).toEqual(["$75.50", "+3.02R", "1 : 4.00", value(model, "Rule adherence"), "3 / 10 checks confirmed", "4/5"]);
    expect(model.kpis.map(kpi => kpi.tone)).toEqual(["positive", "positive", "accent", "neutral", "neutral", "neutral"]);
    const losing = buildTradePdfModel({ ...completeTrade, pnl: "-30.00", risk: "25.00" });
    expect(losing.kpis[0]).toEqual({ label: "Actual P&L", value: "-$30.00", tone: "negative" });
    expect(losing.kpis[1].tone).toBe("negative");
  });

  it("publishes the checklist as report-ready rows, keeping unconfirmed and unknown items", () => {
    const model = buildTradePdfModel(completeTrade);
    expect(model.checklist).toHaveLength(10);
    expect(model.checklist.filter(item => item.confirmed).map(item => item.label)).toEqual(["Setup exists", "Entry condition confirmed", "Risk is within limit"]);
    expect(model.checklist.filter(item => !item.recorded)).toHaveLength(7);
    expect(model.checklist[1]).toEqual({ label: "Setup matches today's plan", confirmed: false, recorded: false });
    // A checklist saved as objects, including an id outside the current gate.
    const objectShape = buildTradePdfModel({ id: 1, planChecklist: [{ id: "setup-exists", label: "Setup exists", checked: true }, { id: "legacy-check", label: "Legacy check", checked: false }] });
    expect(objectShape.checklist.filter(item => item.recorded)).toEqual([
      { label: "Setup exists", confirmed: true, recorded: true },
      { label: "Legacy check", confirmed: false, recorded: true },
    ]);
    expect(objectShape.checklist).toHaveLength(11);
  });

  it("describes the process independently of the outcome", () => {
    const goodWin = buildTradePdfModel({ id: 1, tradeDate: "2026-08-12", result: "WIN", pnl: "10", risk: "10", planStatus: "PLANNED", planChecklist: "setup-exists|matches-plan|entry-confirmed|stop-defined|risk-in-limit|size-valid|rr-acceptable|session-valid|no-emotional-trigger|not-revenge", setupQuality: "A", risk: "10" });
    expect(goodWin.classification.key).toBe("GOOD_WIN");
    expect(goodWin.classification.tone).toBe("positive");
    const badLoss = buildTradePdfModel({ id: 2, tradeDate: "2026-08-12", result: "LOSS", pnl: "-10", risk: "10", mistake: "Moved SL|Oversize" });
    expect(badLoss.classification.key).toBe("BAD_LOSS");
    expect(badLoss.classification.tone).toBe("negative");
    const untouched = buildTradePdfModel({ id: 3, tradeDate: "2026-08-12", result: "", pnl: null });
    expect(untouched.classification.key).toBe("NOT_EVALUATED");
    expect(untouched.classification.tone).toBe("neutral");
    // The badge never lets a profitable outcome stand in for a good process.
    expect(buildTradePdfModel({ id: 4, tradeDate: "2026-08-12", result: "WIN", pnl: "50", risk: "10", mistake: "Revenge|Oversize" }).classification.key).toBe("BAD_WIN");
  });
  ...
  it("renders a missing optional field as — rather than dropping it", () => {
    ...
    expect(labels(minimal)).toEqual(expect.arrayContaining(["Trade ID", "Trade date", "Symbol", "MT5 ticket", "Session", "Direction", "Result", "Timeframe", "Level / confluence", "Mistake tags", "Planned risk", "Actual P&L", "Running balance", "Plan status", "Checklist completion", "Confirmation"]));
    ["Symbol", "MT5 ticket", "Session", "Direction", "Timeframe", "Planned risk", "Actual P&L", "Running balance", "Checklist completion"].forEach(label => expect(value(minimal, label), `${label} should render the missing marker`).toBe(PDF_MISSING));
    expect(value(minimal, "Result")).toBe(PDF_MISSING);
    // Missing figures never claim a direction; the R:R label keeps its accent.
  ...
  it("exports plan status and the pre-trade checklist state", () => {
    ...
    expect(model.checklist[0]).toEqual({ label: "Setup exists", confirmed: true, recorded: true });
    expect(model.checklist.find(item => item.label === "Stop-loss defined")).toEqual({ label: "Stop-loss defined", confirmed: false, recorded: true });
    expect(value(model, "Checklist completion")).toBe("1 / 10 checks confirmed");
  });

  it("flags the values the compact report must give the full column width", () => {
    const model = buildTradePdfModel(completeTrade);
    const field = (label: string) => fields(model).find(entry => entry.label === label);
    expect(field("Level / confluence")?.wide).toBe(true);
    expect(field("Process review")?.wide).toBe(true);
    expect(field("Trade ID")?.wide).toBeUndefined();
    // Tones are declared on the model, not guessed by the renderer.
    expect(field("Actual P&L")?.tone).toBe("signed");
    expect(field("Planned R:R")?.tone).toBe("accent");
    expect(fieldTone(field("Actual P&L")!)).toBe("positive");
    expect(fieldTone({ tone: "signed", value: "-$12.00" })).toBe("negative");
    expect(fieldTone({ tone: "signed", value: PDF_MISSING })).toBe("neutral");
  });
  ...
  it("labels an unsynced local record and an empty checklist honestly", () => {
    ...
    expect(checklistItems(null).every(item => item.recorded === false)).toBe(true);
