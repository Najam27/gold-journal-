...
export function AccountRenameControl() {
  ...
  const accountsQuery = trpc.accounts.list.useQuery(undefined, { enabled: Boolean(privateReady), retry: false, refetchOnWindowFocus: false, staleTime: 60_000 });
  const accounts = (accountsQuery.data ?? []) as Array<{ id: number; name: string; startingBalance: unknown; createdAt: unknown; updatedAt: unknown }>;

  const account = accounts.find((item: any) => item.id === selectedAccountId) ?? accounts[0];
  ...
  useEffect(() => {
    ...
    const validId = selectedAccountId && accounts.some(item => item.id === selectedAccountId) ? selectedAccountId : accounts[0]?.id;
    if (validId && validId !== selectedAccountId) { setLocalSelectedAccountId(validId); setSelectedAccountId(validId); }
  }, [accounts, privateReady, selectedAccountId]);
  ...
  const addAccount = async () => {
    ...
    try { const result = await create.mutateAsync({ name: trimmed, startingBalance: 0 }); setNewName(""); setLocalSelectedAccountId(result.id); setSelectedAccountId(result.id); await refresh(); toast.success("New trading account created."); } catch (error: any) { toast.error(error.message || "Account could not be created."); }
  };
