...
import { ClipboardCheck, ImagePlus, ListChecks, Plus, ShieldCheck } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { executionTypes, formatActualR, formatRr, levels, results, sessions } from "@/lib/gold";
import {
...
function CustomSelect({ category, value, onChange, options, placeholder, multi = false }: { category: string; value: string; onChange: (value: string) => void; options: string[]; placeholder?: string; multi?: boolean }) {
  ...
  const add = async () => {
    ...
    if (!next) return;
    try { await addOption.mutateAsync({ category, value: next }); await utils.optionLists.list.invalidate(); onChange(next); setDraft(""); toast.success(`${category} saved for future trades.`); }
    catch (error: any) { toast.error(error.message || `Could not save ${category}.`); }
  };
  const addTag = async () => {
    const next = draft.trim();
    if (!next) return;
    try { await addOption.mutateAsync({ category, value: next }); await utils.optionLists.list.invalidate(); onChange(selectedTags.some(item => item.toLowerCase() === next.toLowerCase()) ? selectedTags.join(" | ") : [...selectedTags, next].join(" | ")); setDraft(""); toast.success(`${category} option saved for future trades.`); }
    catch (error: any) { toast.error(error.message || `Could not save ${category} option.`); }
  };
 ...
 *
 * The tags are written back into the existing `mistake` column as a pipe-separated
 * string, so historic tags, saved custom options, and goal rules keep working.
 */
function MistakeTaxonomy({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const [draft, setDraft] = useState("");
  const addOption = trpc.optionLists.add.useMutation();
  const utils = trpc.useUtils();
  const selected = value.split(/[|,;]+/).map(tag => tag.trim()).filter(Boolean);
  ...
  const toggle = (label: string) => onChange(isSelected(label) ? selected.filter(tag => tag.toLowerCase() !== label.toLowerCase()).join(" | ") : [...selected, label].join(" | "));
  const addTag = async () => {
    const next = draft.trim();
    if (!next) return;
    try { await addOption.mutateAsync({ category: "Mistake", value: next }); await utils.optionLists.list.invalidate(); onChange(selected.some(item => item.toLowerCase() === next.toLowerCase()) ? selected.join(" | ") : [...selected, next].join(" | ")); setDraft(""); toast.success("Mistake option saved for future trades."); }
    catch (error: any) { toast.error(error.message || "Could not save the mistake option."); }
  };
  const customs = Array.from(new Set(selected.filter(tag => !MISTAKE_TAXONOMY.some(item => item.label.toLowerCase() === tag.toLowerCase()))));
  return <div className="taxonomy-wrap">{TAXONOMY_CATEGORIES.map(category => <div className="taxonomy-category" key={category}><span>{MISTAKE_CATEGORY_LABELS[category].toUpperCase()}</span><div className="taxonomy-group">{MISTAKE_TAXONOMY.filter(item => item.category === category).map(item => <button type="button" key={item.tag} className={isSelected(item.label) ? "selected" : ""} aria-pressed={isSelected(item.label)} onClick={() => toggle(item.label)}>{item.label}</button>)}</div></div>)}{customs.length > 0 && <div className="taxonomy-category"><span>YOUR OWN TAGS</span><div className="taxonomy-group">{customs.map(tag => <button type="button" key={tag} className="selected" aria-pressed onClick={() => toggle(tag)}>{tag}</button>)}</div></div>}<div className="journal-value-add"><Input aria-label="Add custom Mistake option" value={draft} maxLength={70} onChange={event => setDraft(event.target.value)} placeholder="+ Add custom mistake tag" /><button type="button" title="Save custom Mistake option" aria-label="Save custom Mistake option" disabled={addOption.isPending || !draft.trim()} onClick={() => void addTag()}><Plus size={14} /></button></div><p className="gate-footer">Only tag what actually happened. Untagged trades are treated as unevaluated, never as a breach.</p></div>;
}
...
export function TradeDialogWithCustomOptions({ open, setOpen, form, setForm, editing, onSave, pending, screenshot, setScreenshot, progress, plans, dayTrades, behaviorConfig }: any) {
  ...
  </div><div className="dialog-actions"><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={pending} onClick={onSave}>{pending ? "Saving…" : editing ? "Save changes" : "Save trade"}</Button></div></DialogContent></Dialog>;
}
