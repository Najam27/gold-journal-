...
export function BulkPdfExporter() {
  ...
  const createPdf = async () => {
    ...
    try {
      ...
      sessions.forEach((session, index) => { const rows = reportSelected.filter((trade: any) => (trade.session || "Unspecified") === session); const pnl = rows.reduce((sum: number, trade: any) => sum + toNumber(trade.pnl), 0); const y = 58 + index * 11; const pnlColor = pnl >= 0 ? colors.green : colors.red; pdf.setTextColor(...colors.text); pdf.setFontSize(10); pdf.text(session, page.margin, y); pdf.setTextColor(pnlColor[0], pnlColor[1], pnlColor[2]); pdf.text(formatMoney(pnl), 115, y); pdf.setTextColor(...colors.muted); pdf.text(`${rows.length} trade${rows.length === 1 ? "" : "s"}`, 155, y); });
