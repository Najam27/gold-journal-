...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    mt5: {
      ...
      integrity: {
        useQuery: () => ({
          data: { health: { label: "Live MT5 sync" }, findings: [] },
          isFetching: false,
        }),
      },
      createConnection: {
        useMutation: () => ({ mutateAsync: mocks.create, isPending: false }),
      },
      replaceConnection: {
      ...
      setConnectionActive: {
        useMutation: () => ({ mutateAsync: mocks.setActive }),
      },
      deleteConnection: { useMutation: () => ({ mutateAsync: mocks.remove }) },
    },
    useUtils: () => ({
      mt5: {
        workspace: { invalidate: mocks.invalidate },
...
describe("Mt5LiveView", () => {
  ...
  it("shows masked credentials, account metrics, historical positions, the deployment-specific EA, and automatic Trade Log synchronization", () => {
    const onJournalNow = vi.fn();
    render(
      <Mt5LiveView
        account={{ id: 12, name: "GFT 10K" }}
        accounts={[
          { id: 12, name: "GFT 10K" },
          { id: 13, name: "FundingPips" },
        ]}
        onJournalNow={onJournalNow}
      />
    );
    expect(screen.getByText("GFT Live")).toBeTruthy();
    expect(
      screen.queryByText("mt5_secret_connection_key_abcdefghijk")
    ).toBeNull();
    ...
    expect(
      screen.getByRole("columnheader", { name: "Trade Log" })
    ).toBeTruthy();
    ...
    expect(
      (
        screen.getByRole("combobox", {
          name: /Broker UTC offset for GFT Live/i,
  ...
  it("generates a replacement key directly when MT5 history exists but the connection record is missing", async () => {
    ...
    mocks.historyData = {
      positions: mocks.workspaceData.closedPositions,
      total: 1,
      page: 1,
      pageSize: 20,
      pageCount: 1,
  ...
  it("shows setup diagnostics while waiting for the first EA contact", () => {
    mocks.workspaceData = {
      connections: [
        {
          id: 1,
    ...
    expect(screen.getByText(/Waiting for MT5 account metrics/i)).toBeTruthy();
