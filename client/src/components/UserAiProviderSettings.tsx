...
export function UserAiProviderSettings() {
  ...
  return <section className="panel ai-provider-settings"><span className="section-label">PRIVATE AI PROVIDER</span><h3><KeyRound size={17} /> OpenRouter key</h3><p>Your key is sent only to the authenticated server, encrypted before database storage, and never returned to this browser after save. AI Analysis, AI Mentor, and Risk Coach use this key.</p>{vaultUnavailable ? <p className="ai-key-vault-warning" role="alert">Key testing is available, but saving is disabled because this deployment has no server-only encryption material. Configure a server encryption secret and retry.</p> : status.data?.configured ? <div className="ai-key-status"><ShieldCheck size={16} /><span>Connected as <b>{status.data.maskedKey}</b> · {status.data.model}</span></div> : <p className="muted">No AI key connected. Add your personal OpenRouter key to enable AI features.</p>}<div className="ai-provider-form"><Input type="password" autoComplete="off" value={key} onChange={event => setKey(event.target.value)} placeholder="sk-or-v1-…" aria-label="OpenRouter API key" disabled={busy} /><Input value={model} onChange={event => setModel(event.target.value)} placeholder="OpenRouter model" aria-label="OpenRouter model" disabled={busy} /><div className="dialog-actions"><Button variant="outline" disabled={busy || key.trim().length < 20} onClick={validate}>{testKey.isPending && <Loader2 className="animate-spin" size={15} />} Test key</Button><Button disabled={vaultUnavailable || busy || key.trim().length < 20 || !model.trim()} onClick={save}>{saveKey.isPending && <Loader2 className="animate-spin" size={15} />} {status.data?.configured ? "Replace key" : "Save key"}</Button>{status.data?.configured && <Button variant="outline" className="danger-button" disabled={vaultUnavailable || busy} onClick={remove}><Trash2 size={15} /> Delete</Button>}</div></div></section>;
}
