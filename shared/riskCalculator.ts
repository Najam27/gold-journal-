...
export type RiskCalculation = { valid: boolean; basis: RiskBasis; capital: number; freeMargin: number; riskPercent: number; riskAmount: number; stopDistance: number; stopTicks: number; lossPerLot: number; rawLots: number; lots: number; actualRisk: number; symbol: string | null; currency: string | null; warnings: string[]; verification: string[] };
...
export function calculateRisk(input: RiskInput, account: AccountRiskSnapshot | null, spec: BrokerRiskSpec | null): RiskCalculation {
  ...
  const base = { valid: false, basis: input.basis, capital: round(capital, 2), freeMargin: round(account?.freeMargin ?? 0, 2), riskPercent: round(input.riskPercent, 4), riskAmount: 0, stopDistance: 0, stopTicks: 0, lossPerLot: 0, rawLots: 0, lots: 0, actualRisk: 0, symbol: spec?.symbol ?? null, currency: account?.currency ?? null, warnings, verification };
  ...
  return { ...base, valid: lots >= spec.volumeMin && warnings.length === 0, riskAmount: round(riskAmount, 2), stopDistance: round(stopDistance), stopTicks: round(stopTicks), lossPerLot: round(lossPerLot, 4), rawLots: round(rawLots), lots: round(lots), actualRisk: round(actualRisk, 2) };
}
