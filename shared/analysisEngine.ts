...
export type AnalysisResult = {
  ...
  execution: { plannedRAvailable: number; actualRAvailable: number; targetCaptureAvailable: number; averagePlannedR: number | null; averageActualR: number | null; averageTargetCapture: number | null; medianTargetCapture: number | null; reachedOrExceededTarget: number; profitableBelowTarget: number; nonPositiveOutcome: number; message: string };
...
function executionMetrics(trades: AnalysisTrade[]) {
  ...
  return {
    ...
    averageActualR: actualRs.length ? round(actualRs.reduce((sum, value) => sum + value, 0) / actualRs.length, 4) : null,
    averageTargetCapture: captureRows.length ? round(captureRows.reduce((sum, row) => sum + row.capture, 0) / captureRows.length, 2) : null,
    medianTargetCapture: captureRows.length ? round(median(captureRows.map(row => row.capture)), 2) : null,
    reachedOrExceededTarget: captureRows.filter(row => row.actualR >= row.plannedR).length,
    profitableBelowTarget: captureRows.filter(row => row.actualR > 0 && row.actualR < row.plannedR).length,
    nonPositiveOutcome: captureRows.filter(row => row.actualR <= 0).length,
    message: captureRows.length ? "Target capture compares realized P&L with the reward saved before the trade. It shows outcome relative to the planned target; it does not infer why a target was changed or a trade was exited." : "Target capture is unavailable until closed trades include positive planned risk and reward values."
  };
}
