...
export type EvidenceTier = "INSUFFICIENT DATA" | "EARLY SIGNAL" | "DEVELOPING" | "REPEATABLE" | "STRONGER EVIDENCE";
...
function evidenceTier(sample: number): EvidenceTier { if (sample < 5) return "INSUFFICIENT DATA"; if (sample < 10) return "EARLY SIGNAL"; if (sample < 20) return "DEVELOPING"; if (sample < 30) return "REPEATABLE"; return "STRONGER EVIDENCE"; }
...
function groupedDays(trades: AnalysisTrade[]) { const labels = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]; return groupBy(sortedClosed(trades).map(trade => ({ ...trade, day: labels[new Date(dateValue(trade.tradeDate)).getUTCDay()] })), ["day"]); }
function groupedHours(trades: AnalysisTrade[]) { return groupBy(sortedClosed(trades).map(trade => ({ ...trade, hour: `${String(new Date(dateValue(trade.tradeDate)).getUTCHours()).padStart(2, "0")}:00 UTC` })), ["hour"]); }
...
function riskMetrics(trades: AnalysisTrade[]) { const closed = sortedClosed(trades); const risks = closed.map(t => finite(t.risk)).filter(value => value > 0); const winners = closed.filter(t => t.result === "WIN").map(t => finite(t.risk)).filter(value => value > 0); const losers = closed.filter(t => t.result === "LOSS").map(t => finite(t.risk)).filter(value => value > 0); const drawdownRisks = closed.slice(0, Math.max(0, closed.length - 1)).filter((_, index) => index > 0 && finite(closed[index - 1].pnl) < 0).map(t => finite(t.risk)).filter(value => value > 0); return { available: risks.length, average: risks.length ? round(risks.reduce((sum, value) => sum + value, 0) / risks.length, 2) : null, median: risks.length ? round(median(risks), 2) : null, consistency: risks.length ? round(1 - Math.min(1, Math.sqrt(risks.reduce((sum, value) => sum + (value - risks.reduce((a, b) => a + b, 0) / risks.length) ** 2, 0) / risks.length) / Math.max(1, risks.reduce((a, b) => a + b, 0) / risks.length)), 4) : null, afterWins: winners.length ? round(winners.reduce((sum, value) => sum + value, 0) / winners.length, 2) : null, afterLosses: losers.length ? round(losers.reduce((sum, value) => sum + value, 0) / losers.length, 2) : null, duringDrawdown: drawdownRisks.length ? round(drawdownRisks.reduce((sum, value) => sum + value, 0) / drawdownRisks.length, 2) : null }; }
...
export function buildAnalysis(trades: AnalysisTrade[], filters: AnalysisFilters = {}): AnalysisResult {
  ...
  const warnings = [...quality.warnings.filter(item => item.percentage >= 20).map(item => item.message), ...(closed.length < 20 ? ["Evidence is limited: fewer than 20 closed trades are available."] : []), ...(filtered.length > 0 && closed.length !== filtered.length ? ["OPEN trades are excluded from performance metrics."] : []), ...(!mfeTrades.length ? ["MFE/MAE unavailable: no excursion series is stored for these trades."] : [])];
  return { version: "analysis-v1", filters, timezone: "UTC", period: { start: closed.length ? new Date(dateValue(closed[0].tradeDate)).toISOString() : null, end: closed.length ? new Date(dateValue(closed.at(-1)?.tradeDate)).toISOString() : null, sample: overview.sample }, overview, ...allRows, rolling, decay, streaks: { ...streak, afterWin: afterWins.length ? metricRow("After wins", afterWins) : null, afterLoss: afterLosses.length ? metricRow("After losses", afterLosses) : null }, drawdown: { maximum: drawdown.maxDrawdown, average: drawdown.drawdownCount ? round(drawdown.maxDrawdown / drawdown.drawdownCount, 2) : 0, largest: drawdown.maxDrawdown, count: drawdown.drawdownCount, durationTrades: drawdown.durationTrades, recoveryTrades: drawdown.recoveryTrades },     duration: durationMetrics(filtered), risk: riskMetrics(filtered), winLoss: { winners: metricRow("Winners", closed.filter(trade => trade.result === "WIN")), losers: metricRow("Losers", closed.filter(trade => trade.result === "LOSS")), dimensions: winLossDimensions }, journalQuality: quality, mfeMae: { available: mfeTrades.length, unavailable: closed.length - mfeTrades.length, message: mfeTrades.length ? "Historical excursion fields were available for this sample." : "MFE/MAE unavailable because the journal does not store price-series excursions.", mfe: mfeValues.length ? metricRow("MFE", mfeValues) : null, mae: maeValues.length ? metricRow("MAE", maeValues) : null }, exitEfficiency: { available: false, message: "Historical exit efficiency is unavailable because achievable intratrade maximums are not stored." }, edgeCards: { top: rank([...allRows.sessions, ...allRows.timeframes, ...allRows.levels, ...allRows.setups, ...allRows.sessionTimeframes, ...allRows.levelSessions, ...allRows.levelTimeframes]).find(row => row.sample >= EDGE_MIN_SAMPLE) ?? null, weak: rank([...allRows.sessions, ...allRows.timeframes, ...allRows.levels, ...allRows.setups, ...allRows.sessionTimeframes, ...allRows.levelSessions, ...allRows.levelTimeframes].filter(row => row.sample >= EDGE_MIN_SAMPLE).sort((a, b) => a.edgeScore - b.edgeScore))[0] ?? null, mostConsistent: rank([...allRows.sessions, ...allRows.timeframes, ...allRows.levels].filter(row => row.sample >= 10).sort((a, b) => confidenceRank[b.confidence] - confidenceRank[a.confidence] || b.dataCompleteness - a.dataCompleteness))[0] ?? null, highestExpectancy: rank([...allRows.sessions, ...allRows.timeframes, ...allRows.levels, ...allRows.setups].filter(row => row.sample >= EDGE_MIN_SAMPLE).sort((a, b) => b.expectancy - a.expectancy))[0] ?? null, bestR: rank([...allRows.sessions, ...allRows.timeframes, ...allRows.levels, ...allRows.setups].filter(row => row.sample >= EDGE_MIN_SAMPLE && row.expectancyR !== null).sort((a, b) => (b.expectancyR ?? -Infinity) - (a.expectancyR ?? -Infinity)))[0] ?? null }, warnings };
}
